...
certs=$(curl -s https://open.plugncharge-test.hubject.com/.well-known/cpo/cacerts \
  -H 'Accept: application/pkcs10, application/json' \
  -H "Authorization: Bearer ${BEARER_TOKEN}" \
  -H 'Content-Transfer-Encoding: application/pkcs10' | openssl enc -base64 -d | openssl pkcs7 -inform DER -print_certs)
echo "${certs}" | awk '/subject.*CN.*=.*CPO Sub1 CA QA G1.3/,/END CERTIFICATE/' >"${script_dir}"/../config/certificates/cpo_sub_ca1.pem
echo "${certs}" | awk '/subject.*CN.*=.*CPO Sub2 CA QA G1.3.1/,/END CERTIFICATE/' >"${script_dir}"/../config/certificates/cpo_sub_ca2.pem
